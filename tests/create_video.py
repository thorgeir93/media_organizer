...
def add_creation_date_to_video(video_path: str, date_str: str):
    # This uses ffmpeg to add metadata to the video
    cmd = ["ffmpeg", "-i", video_path, "-metadata", f"creation_time={date_str}",
        "-codec", "copy", f"{video_path}_with_date.mp4"]
    subprocess.run(cmd)
