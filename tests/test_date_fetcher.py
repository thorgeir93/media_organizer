...
from .create_img import create_test_image_with_date
...
def test_image_creation_date_extraction(tmpdir):
    ...
    create_test_image_with_date(image_path, "2023:05:20 15:45:50")
...
def test_video_creation_date_extraction(tmpdir):
    ...
    create_test_video(str(video_path))   # Convert LocalPath to string
    add_creation_date_to_video(video_path, "2023-05-20T15:45:50")

    # Extract the date using your function
    extracted_date = get_accurate_media_date(Path(str(video_path + "_with_date.mp4")))  # Convert LocalPath to string
    assert extracted_date == datetime(2023, 5, 20, 15, 45, 50)
